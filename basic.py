...
TT_LUTAW = 'LUTAW'
TT_SULAT = 'SULAT'
TT_TIGPAILA = 'TIGPAILA'
TT_SUSIPULONG = 'SUSIPULONG'
TT_DUGANG = 'DUGANG'
TT_TANGGAL = 'TANGGAL'
...
class Lexer:
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                tokens.append(Token(TT_BAGONGLINYA, pos_start=self.pos))
                self.advance()
                ...
                tokens.append(Token(TT_BAHINON, pos_start=self.pos))
                self.advance()
            elif self.current_char == '^':
                tokens.append(Token(TT_GAHOM, pos_start=self.pos))
                self.advance()
            elif self.current_char == '(':
    ...
    def make_number(self):
        ...
        if dot_count == 0:
            return Token(TT_NUMERO, int(num_str), pos_start, self.pos)
        else:
            return Token(TT_LUTAW, float(num_str), pos_start, self.pos)
    ...
    def make_string(self):
        ...
        self.advance()
        return Token(TT_SULAT, string, pos_start, self.pos)
    ...
    def make_identifier(self):
        ...
        tok_type = TT_SUSIPULONG if id_str in KEYWORDS else TT_TIGPAILA
...
class Parser:
    ...
    def statements(self):
        ...
        while self.current_tok.type == TT_BAGONGLINYA:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_SUSIPULONG, 'PADAYON'):
            res.register_advancement()
            self.advance()
            return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))

        if self.current_tok.matches(TT_SUSIPULONG, 'PUTOL'):
            res.register_advancement()
            self.advance()
    ...
    def comp_expr(self):
        ...
        if self.current_tok.matches(TT_SUSIPULONG, 'DILI'):
    ...
    def arith_expr(self):
        return self.bin_op(self.term, (TT_DUGANG, TT_TANGGAL))

    def term(self):
        return self.bin_op(self.factor, (TT_DAGHANON, TT_BAHINON))

    def factor(self):
        res = ParseResult()
        tok = self.current_tok

        if tok.type in (TT_DUGANG, TT_TANGGAL):
            res.register_advancement()
            self.advance()
    ...
    def power(self):
        return self.bin_op(self.call, (TT_GAHOM,), self.factor)
    ...
    def atom(self):
        ...
        if tok.type in (TT_NUMERO, TT_LUTAW):
            res.register_advancement()
            self.advance()
            return res.success(NumberNode(tok))

        elif tok.type == TT_SULAT:
            res.register_advancement()
            self.advance()
            return res.success(StringNode(tok))

        elif tok.type == TT_TIGPAILA:
            res.register_advancement()
            self.advance()
    ...
    def if_expr_b(self):
        return self.if_expr_cases('PUWEDEPUD')

    def if_expr_c(self):
        res = ParseResult()
        else_case = None

        if self.current_tok.matches(TT_SUSIPULONG, 'KUNGDILI'):
            res.register_advancement()
            self.advance()
    ...
    def func_def(self):
        ...
        if self.current_tok.type == TT_PANA:
            res.register_advancement()
            self.advance()
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        elif node.op_tok.type == TT_TANGGAL:
            result, error = left.subbed_by(right)
        elif node.op_tok.type == TT_DAGHANON:
            result, error = left.multed_by(right)
        elif node.op_tok.type == TT_BAHINON:
            result, error = left.dived_by(right)
        elif node.op_tok.type == TT_GAHOM:
        ...
        elif node.op_tok.matches(TT_SUSIPULONG, 'UG'):
            result, error = left.anded_by(right)
        elif node.op_tok.matches(TT_SUSIPULONG, 'O'):
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_TANGGAL:
            number, error = number.multed_by(Number(-1))
        elif node.op_tok.matches(TT_SUSIPULONG, 'DILI'):
...
global_symbol_table.set("WAY_SULOD", Number.null)
...
global_symbol_table.set("IBUTANG", BuiltInFunction.input)
global_symbol_table.set("IBUTANG_NUMERO", BuiltInFunction.input_int)
global_symbol_table.set("HINLO", BuiltInFunction.clear)
